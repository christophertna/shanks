...
| [`../hooks/test.hooks/test-prompt-repo-state.sh`](../hooks/test.hooks/test-prompt-repo-state.sh) | 23 shell checks | `UserPromptSubmit` repository-state injection: branch, upstream drift, uncommitted changes (with truncation), recent commits, and the cached open-PR line, plus the fail-open paths - non-repository directory, missing `git`, broken `gh`, and detached HEAD - each of which must still exit zero |
