...
files. The Python suite contains **222 unittest methods** across twelve
...
| --- | ---: | --- |
| [`test_cli.py`](test_cli.py) | 32 | Execution-mode reporting, dev worktree creation and refresh (carrying `.claude/settings.json` and `.claude/skills/` in, leaving `settings.local.json` out) with ignored `.venv` symlinks, doctor diagnostics (including Git/gh/gitleaks version floors, `core.hooksPath`, unwired Claude Code hooks, and stale worktree hook settings), documentation-count guards for the Python and shell test suites (including the shared count derivation and its fail-closed harness runner), hook-harness coverage in this file and in CI, and run listing, status (including pending interrupts, drift, and recent events), resume, cancellation, recovery, cleanup, pruning, and safety checks |
...
| [`../hooks/test.hooks/test-prompt-repo-state.sh`](../hooks/test.hooks/test-prompt-repo-state.sh) | 36 shell checks | `UserPromptSubmit` repository-state injection: branch, upstream drift (printed even at zero, with the last successful fetch's age under both BSD and GNU `stat`, and the truncated-`FETCH_HEAD` case), the debounced background fetch and its skip inside the debounce window, uncommitted changes (with truncation), recent commits, and the cached open-PR and `main`-CI lines, plus the fail-open paths - non-repository directory, missing `git`, broken `gh`, and detached HEAD - each of which must still exit zero |
