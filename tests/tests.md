...
| [`../hooks/test.hooks/test-prompt-repo-state.sh`](../hooks/test.hooks/test-prompt-repo-state.sh) | 34 shell checks | `UserPromptSubmit` repository-state injection: branch, upstream drift (printed even at zero, with the last successful fetch's age and the truncated-`FETCH_HEAD` case), the debounced background fetch and its skip inside the debounce window, uncommitted changes (with truncation), recent commits, and the cached open-PR and `main`-CI lines, plus the fail-open paths - non-repository directory, missing `git`, broken `gh`, and detached HEAD - each of which must still exit zero |
