...
## Corpus Check
- 49 files · ~44,739 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 952 nodes · 2481 edges · 49 communities (42 shown, 7 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 229 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `824bf4a7`
...
Cohesion: 0.08
Nodes (25): Command, build_graph(), GraphRoutingTests, _initial_state(), invoke_with_approvals(), PreflightOnlyRepository, Keep the legacy test-double API covered while the graph splits handoffs., Keep graph tests isolated from the shared production checkpoint store. (+17 more)
...
Cohesion: 0.08
Nodes (32): GitHubIntegrationTests, Path, FakeWorkspaceManager, Path, RecordingAdapter, WorkspaceTests, _command_error(), current_workspace_directory() (+24 more)
...
Cohesion: 0.14
Nodes (9): _attach_commands(), _pull_request_names(), _pull_request_number(), Push and open a PR for callers that do not need approval boundaries., Extract case-insensitive label or reviewer names from gh JSON., Extract the number from a GitHub pull-request URL., Keep the full command trail when a multi-command operation fails., Describe the branch push without contacting the remote. (+1 more)
...
Cohesion: 0.13
Nodes (16): Protocol, CheapCriticAdapter, ClaudeAdapter, CodexAdapter, _command_path_arguments(), _path_within_any(), _preflight_failure(), Path (+8 more)
...
Cohesion: 0.14
Nodes (10): _agent_environment(), _audit_command(), _critic_request(), _estimate_tokens(), Keep GitHub credentials out of agent and test subprocesses., Return a redacted command suitable for the persisted run manifest., Add the shared read-only audit instructions to a critic request., Use a conservative four-characters-per-token estimate for CLI text. (+2 more)
...
### Community 24 - "._setup"
Cohesion: 0.11
Nodes (19): dependencies(), LifecycleTests, LeaseLostError, _lifecycle_status(), RuntimeError, Persistent run leases and lifecycle records., Extend the current owner's lease before another node runs., Record a pause caused by a human interrupt while keeping the lease. (+11 more)
...
Cohesion: 0.12
Nodes (13): _checkpoint_payload(), _dependencies(), ExplodingBuilder, FaultInjectionTests, _initial_state(), LocalTestAdapter, Run a configured CLI backend through the common adapter contract., Normalize a read-only CLI critic into the common agent result shape. (+5 more)

### Community 48 - "DebuggerAdapter"
Cohesion: 0.12
Nodes (16): ValueError, _branch_values(), _critic_result(), _debugger_request(), _debugger_result(), DebuggerAdapter, _parse_json_object(), _policy_values() (+8 more)

## Knowledge Gaps
- **108 isolated node(s):** `graphify-update.sh script`, `guard-dependency-files.sh script`, `RALPH_BASE_DIR`, `RALPH_PROJECT_DIR`, `RALPH_RUN_DIR` (+103 more)
...
- **Why does `NodeDependencies` connect `AgentResult` to `GitHubAdapter`, `RunWorkspaceManager`, `.cleanup`, `graph.py`, `RalphAdapter`, `WorkflowState`, `classify_failure`, `migrate_state`, `ValueError`, `DebuggerAdapter`, `nodes.py`, `create_nodes`, `default_dependencies`, `github_node`, `._setup`, `contracts.py`?**
  _High betweenness centrality (0.090) - this node is a cross-community bridge._
- **Why does `AgentResult` connect `AgentResult` to `GitHubAdapter`, `RunWorkspaceManager`, `graph.py`, `RalphAdapter`, `WorkflowState`, `AgentRequest`, `classify_failure`, `ValueError`, `DebuggerAdapter`, `nodes.py`, `default_dependencies`, `adapters.py`, `github_node`, `contracts.py`?**
  _High betweenness centrality (0.078) - this node is a cross-community bridge._
- **Why does `GitHubAdapter` connect `GitHubAdapter` to `AgentResult`, `RunWorkspaceManager`, `RalphAdapter`, `AgentRequest`, `classify_failure`, `DebuggerAdapter`, `nodes.py`, `default_dependencies`, `adapters.py`?**
