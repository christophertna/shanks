...
## Corpus Check
- 50 files · ~45,567 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 963 nodes · 2512 edges · 48 communities (41 shown, 7 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 232 edges (avg confidence: 0.56)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d1aee3dd`
...
Cohesion: 0.10
Nodes (44): TypedDict, Translate common agent output into shared workflow state fields., state_update_from_result(), _apply_failure_policy(), _audit_result(), building(), critic_auditor(), _current_item() (+36 more)
...
Cohesion: 0.09
Nodes (24): ValueError, _agent_environment(), _branch_values(), CheapCriticAdapter, _command_path_arguments(), _critic_request(), _critic_result(), _debugger_result() (+16 more)
...
Cohesion: 0.14
Nodes (10): _audit_command(), _estimate_tokens(), _format_request(), Return a redacted command suitable for the persisted run manifest., Create a stable prompt envelope for CLI-backed adapters., Use a conservative four-characters-per-token estimate for CLI text., Check tools, branch state, GitHub auth, and the test environment., Describe the commit without staging or committing anything. (+2 more)
...
Cohesion: 0.11
Nodes (23): Reusable workflow state, agent contracts, adapters, and nodes., acceptance_criteria_for_item(), append_run_manifest(), cancel_run(), _migrate_v0_to_v1(), _migrate_v1_to_v2(), _migrate_v2_to_v3(), _migrate_v3_to_v4() (+15 more)
...
Cohesion: 0.08
Nodes (24): Path, Real-CLI smoke tests for the Codex and Claude adapters.  Opt-in and skipped by d, RealAgentSmokeTests, ClaudeAdapter, ClaudeOpus48CriticAdapter, CodexAdapter, GPT56LunaCriticAdapter, Adapter for direct Codex CLI execution. (+16 more)
...
Cohesion: 0.11
Nodes (14): _checkpoint_payload(), ExplodingBuilder, FaultInjectionTests, _initial_state(), _debugger_request(), DebuggerAdapter, LocalTestAdapter, Run a configured CLI backend through the common adapter contract. (+6 more)
...
- **Why does `NodeDependencies` connect `AgentResult` to `RunWorkspaceManager`, `.cleanup`, `graph.py`, `RalphAdapter`, `WorkflowState`, `classify_failure`, `migrate_state`, `state.py`, `nodes.py`, `create_nodes`, `default_dependencies`, `RunLifecycleManager`, `github_node`, `RalphAdapter`, `lifecycle.py`?**
  _High betweenness centrality (0.090) - this node is a cross-community bridge._
- **Why does `AgentResult` connect `AgentResult` to `GitHubAdapter`, `RunWorkspaceManager`, `RalphAdapter`, `WorkflowState`, `AgentRequest`, `classify_failure`, `RunLifecycleManager`, `state.py`, `nodes.py`, `default_dependencies`, `github_node`, `RalphAdapter`?**
  _High betweenness centrality (0.077) - this node is a cross-community bridge._
- **Why does `RunWorkspaceManager` connect `RunWorkspaceManager` to `AgentResult`, `.cleanup`, `cli.py`, `graph.py`, `main`, `ValueError`, `nodes.py`, `create_nodes`, `default_dependencies`?**
