...
## Corpus Check
- 52 files · ~46,524 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 980 nodes · 2545 edges · 53 communities (45 shown, 8 thin omitted)
- Extraction: 91% EXTRACTED · 9% INFERRED · 0% AMBIGUOUS · INFERRED: 232 edges (avg confidence: 0.56)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4bb12ca7`
...
Cohesion: 0.14
Nodes (24): Translate common agent output into shared workflow state fields., state_update_from_result(), _apply_failure_policy(), building(), critic_auditor(), _current_item(), debugger(), _default_plan() (+16 more)
...
Cohesion: 0.11
Nodes (15): _agent_environment(), _critic_request(), _format_request(), RalphAdapter, Run a configured CLI backend through the common adapter contract., Keep GitHub credentials out of agent and test subprocesses., Add the shared read-only audit instructions to a critic request., Create a stable prompt envelope for CLI-backed adapters. (+7 more)
...
Cohesion: 0.12
Nodes (19): RunnableConfig, _debugger_details(), _invalid_budget(), _nonnegative_float(), _nonnegative_int(), Standardized LangGraph node implementations., Format new debugger findings for the current PRD requirement., Return the first configured cancellation or budget stop reason. (+11 more)
...
Cohesion: 0.14
Nodes (17): ClaudeOpus48CriticAdapter, GPT56LunaCriticAdapter, Normalize a read-only CLI critic into the common agent result shape., Run the critic_auditor as a read-only GPT-5.6 Luna Codex subagent., Run the critic_auditor as a read-only Claude Opus 4.8 subagent., StructuredCriticAdapter, AgentAdapter, Interface implemented by Ralph, model CLIs, and test doubles. (+9 more)
...
### Community 48 - "Path"
Cohesion: 0.13
Nodes (20): ValueError, _branch_values(), _command_path_arguments(), _critic_result(), _debugger_result(), _estimate_tokens(), _parse_json_object(), _parse_uncertainties() (+12 more)

### Community 51 - "PRDItem"
Cohesion: 0.22
...
## Knowledge Gaps
- **110 isolated node(s):** `graphify-update.sh script`, `guard-dependency-files.sh script`, `RALPH_BASE_DIR`, `RALPH_PROJECT_DIR`, `RALPH_RUN_DIR` (+105 more)
...
- **Why does `NodeDependencies` connect `AgentResult` to `GitHubAdapter`, `RunWorkspaceManager`, `.cleanup`, `graph.py`, `RalphAdapter`, `WorkflowState`, `AgentRequest`, `RunLifecycleManager`, `migrate_state`, `nodes.py`, `create_nodes`, `default_dependencies`, `RunLifecycleManager`, `github_node`, `DebuggerAdapter`, `PRDItem`, `lifecycle.py`?**
  _High betweenness centrality (0.088) - this node is a cross-community bridge._
- **Why does `RunWorkspaceManager` connect `RunWorkspaceManager` to `AgentResult`, `.cleanup`, `cli.py`, `graph.py`, `main`, `.run`, `nodes.py`, `default_dependencies`, `create_nodes`?**
  _High betweenness centrality (0.076) - this node is a cross-community bridge._
- **Why does `AgentResult` connect `AgentResult` to `GitHubAdapter`, `RunWorkspaceManager`, `graph.py`, `RalphAdapter`, `WorkflowState`, `classify_failure`, `RunLifecycleManager`, `migrate_state`, `Path`, `nodes.py`, `default_dependencies`, `DebuggerAdapter`, `PRDItem`, `github_node`, `contracts.py`?**
