...
| `prompt-repo-state.sh` | UserPromptSubmit | n/a | Prints the repository's current state - branch, ahead/behind its upstream with the age of the last successful fetch (or, for a branch with no upstream, how many commits it holds that `origin/main` does not - so unpushed work is never mistaken for an empty branch), uncommitted changes (truncated, flagged as possibly another agent's), the last few commits, whether this branch has an open PR, and whether `main`'s latest CI run is green - which Claude Code adds to that turn's context, so a session never starts blind about what it is sitting on. The two `gh` calls (`pr list`, `run list`) are the only network work in the foreground and are cached together per branch for 5 minutes (`SHANKS_PROMPT_STATE_CACHE_MINUTES`); a `git fetch` runs detached in the background at most every 5 minutes (`SHANKS_PROMPT_STATE_FETCH_MINUTES`, `0` disables) so the next prompt's counts are current without this one waiting. The test suite is deliberately left out - ~15s would be paid on every prompt | Fails open (prints nothing, exits 0) if `git`/`jq`/`gh` are missing or fail |
